...
| `vue-baseline-e2e.spec.js` | 现行基线端到端回归套件:**41 条** `[FE-XXX-NN]` 用例,按 9 个模块 `test.describe` 分组 |
...
| `FE-BRD` | 01–11 | 默认故事地图(4 切片含 Sprint 4+)、血缘徽章 `▣ x/y · %`、新建→删除、编辑 Sprint/负责人往返、负责人筛选、点卡详情 + 变更日志(含 `<time>` 时间列非空)、**改负责人级联同步未完成子任务**(逐字段还原)、**总览/详细信息密度切换**(37 条一条不丢、每格一条一行、标题零截断、整图 2730px → 547px)、**马甲四图层**(单图层 + 图例写明规则 + 计数守恒 + 颜色之外的字符标记)、**格内三种排序**、**紧急度随「今天」推进** |
...
| `vue-baseline-e2e.spec.js` | **现行**:41 条 `[FE-XXX-NN]` 现行基线回归用例 |
| `run-all.ps1` | **现行**:一体化编排(后端契约 JUnit 128 → 前端 E2E 41 → 前端验收 e2e:verify 8) |
| `测试用例设计_当前基线_v3.md` | **现行**:当前基线用例设计 177 条(契约层 128 + 前端 `[FE-XXX-NN]` 41 + 验收层 8 + 缺口与缺陷) |
