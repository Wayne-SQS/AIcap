...
| `FE-BRD` | 01–11 | 默认故事地图(4 切片含 Sprint 4+)、血缘徽章 `▣ x/y · %`、新建→删除、编辑 Sprint/负责人往返、负责人筛选、点卡详情 + 变更日志(含 `<time>` 时间列非空)、**改负责人级联同步未完成子任务**(逐字段还原)、**总览/详细信息密度切换**(37 条一条不丢、整图 2730px → 一屏)、**马甲四图层**(单图层 + 图例写明规则 + 计数守恒 + 颜色之外的字符标记)、**格内三种排序**、**紧急度随「今天」推进** |
