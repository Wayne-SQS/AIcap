...
**会议 Agent 和真实模型调用尚未实现。** 在线页面提供明确标记的手动建议录入；外部 Agent 可以调用下列接口提交待审建议。origin=agent 只是提交方声明，不是模型调用真实性证明。
...
打开 http://127.0.0.1:8090/index.html。浏览器询问本地网络访问时允许访问本机后端。部署/复制前端时需同时包含 index.html 与 meeting-review.js。
