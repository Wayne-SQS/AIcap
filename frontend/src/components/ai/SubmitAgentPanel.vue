...
/* 任务提交智能体:演示为样例数据;在线时生成按钮禁用(对齐 meeting-review.js)
   离线生成建议送审对齐旧版 renderSubmitAgent,并补只读角色拦截 */
...
const commits = [
  ['a363255', '李锐铭', '部署爱管理像素风团队工作台网站', 'T15'],
  ['7c2d73c', '高思晗', '初始化仓库与 README', 'T01'],
  ['f0e1d2c', '罗子涵', 'UML 用例图渲染修复', 'T08'],
  ['b3a4f5e', '罗子涵', '时序图消息箭头优化', 'T08'],
  ['c9d8e7f', '李锐铭', '看板拖拽状态持久化', 'T04']
]
...
function genDemoSuggestion() {
  ...
  review.suggestions.unshift({
    ...
    evidence: '孙秋实 近 3 天无 commit 且 T07 甘特与成员任务图未关闭', affected: 'T07 · 孙秋实',
...
<template>
  ...
  <div class="sum-grid">
    <div class="sum-card"><h4>🧾 成员工作状态（事实）</h4><ul><li>孙秋实 近 3 天无可见提交</li><li>罗子涵 昨日 6 次提交集中在 T08</li><li>李锐铭 / 高思晗 活动稳定</li></ul></div>
    <div class="sum-card"><h4>⚠ 推断与风险</h4><ul><li>孙秋实 可能被接口联调阻塞（待确认）</li><li>罗子涵 单点负载偏高</li></ul></div>
  </div>
