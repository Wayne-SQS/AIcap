...
运行、接口和 Agent 对接样例见 [会议建议审核 MVP：运行与接入](docs/会议建议审核MVP_运行与接入.md)。前端部署需同时包含 `index.html`、`meeting-review.js` 和 `meeting-agent.js`。
...
**待办(阶段 2+)**:
- 会议 Agent 真实模型评测与音频转写；审核中心继续扩展任务/故事动作
- 任务提交智能体 + GitHub 事件同步
