...
已接通会议转写保存、待审建议提交、管理员/负责人采纳或拒绝、采纳后新增需求池及服务器审核记录。包含提交幂等和重复批准保护；在线页面明确标记手动录入，**尚未接入真实会议 Agent/模型**。

运行、接口和 Agent 对接样例见 [会议建议审核 MVP：运行与接入](docs/会议建议审核MVP_运行与接入.md)。前端部署需同时包含 `index.html` 和 `meeting-review.js`。
...
**待办(阶段 2+)**:
- 会议智能体真实解析与工具调用、音频转写；审核中心继续扩展任务/故事动作和修改后采纳
- 任务提交智能体 + GitHub 事件同步
